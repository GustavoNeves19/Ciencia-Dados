...
-  [Classificação da Qualidade de Frutas](./qualitity-fruit-classification/)
    - **Descrição**: Este projeto foi desenvolvido para aplicar técnicas de aprendizado de máquina na **classificação da qualidade de frutas** com base em características físico-químicas, como tamanho, peso, doçura, crocância, suculência, maturação e acidez. O objetivo é construir modelos capazes de prever se uma fruta pertence à classe **good** ou **bad**, apoiando processos automatizados de controle de qualidade em ambientes de produção. O estudo explorou diversos algoritmos de classificação, com destaque para o **Extra Trees Classifier, Random Forest e XGBoost**, que obtiveram desempenhos robustos. O melhor modelo apresentou **acurácia superior a 90%** e **AUC próxima de 0.96**, evidenciando sua eficácia na tarefa de classificação binária.
    - **Tecnologias**: Scikit-learn, XGBoost, LightGBM, Extra Trees, Random Forest, GridSearchCV, Matplotlib, Seaborn, Pandas, Numpy.
